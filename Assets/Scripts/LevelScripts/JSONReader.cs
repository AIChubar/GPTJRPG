...
/// <summary>
/// Class that parses the JSON representation of the Game World.
/// </summary>
public class JSONReader : MonoBehaviour
{
    ...
    public class NarrativeData
    {
        ...
        public UnitJSON antagonist;
        public UnitGroup antagonistGroup;
    ...
    public class MainCharacter
    {
        ...
        public UnitGroup protagonistGroup;
    }
    [System.Serializable]
    public class Levels
    {
    ...
    {
        public Dictionary<string, UnitTypeCoefficient> unitClass;
    ...
    void Awake()
    {
        ...
        gameWorld.narrativeData.antagonistGroup.units.Insert(0, gameWorld.narrativeData.antagonist);

        string dialoguesFolderPath = Path.Combine(Application.streamingAssetsPath, "Worlds", gameWorldName, "Dialogues");

        // Get all file paths in the directory
        if (Directory.Exists(dialoguesFolderPath))
        {
    ...
    {
        AssignStatsForGroup(gameWorld.narrativeData.antagonistGroup.units, false);
        AssignStatsForGroup(gameWorld.mainCharacter.protagonistGroup.units, true);
    ...
    private void AssignStatsForGroup(List<UnitJSON> units, bool isFriendly)
    {
        ...
        {
            if (!_unitsStats.powerLevelAttributes.TryGetValue(unit.powerLevel, out var unitPowerLevel))
            {
                unit.powerLevel = "strong";
                unitPowerLevel = _unitsStats.powerLevelAttributes[unit.powerLevel];
            }
            if (!_unitsStats.unitClass.TryGetValue(unit.unitClass, out var unitType))
            {
                unit.unitClass = "fighter";
                unitType = _unitsStats.unitClass[unit.unitClass];
            }
